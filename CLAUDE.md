...
- Resolves `false` once the `timeout` elapses (closing the popup then unless `autoClose: false`), after a further 6-second grace period of polling, since the connection may complete moments around the cutoff. The popup reporting itself closed does **not** end polling — a provider whose auth chain serves `Cross-Origin-Opener-Policy: same-origin` severs the window handle, making `closed` read `true` on a tab that is still open, and a severed handle cannot be told apart from a genuinely closed one. So a user who abandons the flow gets `false` at `timeout`, and `timeout: 0` never settles.
